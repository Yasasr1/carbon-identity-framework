 ...
 * for shared user scenarios. When the authenticated user is a shared user, this wrapper
 * returns the tenant domain of the user's resident organization instead of the
 * service provider's tenant domain.
 * <p>
 * All other methods are delegated to the wrapped {@link AuthenticationContext} instance.
 */
public class AuthenticationContextWrapper extends AuthenticationContext {

    private final AuthenticationContext wrappedContext;

    /**
     * Creates a new AuthenticationContextWrapper decorating the given context.
     *
     * @param wrappedContext the original authentication context to wrap.
     */
    public AuthenticationContextWrapper(AuthenticationContext wrappedContext) {
     ...
     * user is a shared user. Otherwise, delegates to the wrapped context.
     *
    ...
    public String getTenantDomain() {
        ...
        /* TODO: when resolving the authenticated user, get the user authenticated in the previous step and check if
            the user was authenticated by the shared user resolver. if so check if the user is shared and proceed.*/
        ...
        for (StepConfig stepConfig : stepMap.values()) {
            if (stepConfig.getAuthenticatedUser() != null && stepConfig.getAuthenticatedUser().isSharedUser() &&
                    "SharedUserIdentifierExecutor".equals(stepConfig.getAuthenticatedAutenticator().getName())) {
    ...
    // --- Delegated methods from MessageContext ---
